...
export function membershipsRealtimeResponseCallback(
  ...
  if (response.events.includes(`teams.${teamID}.memberships.*.create`)) {
    setMemberships((memberships) => {
      console.log(memberships.some((membership) => membership.$id == payloadMembership.$id))
      if (!memberships.some((membership) => membership.$id == payloadMembership.$id)) {
    ...
    setMemberships((memberships) =>
      memberships.filter((membership) => membership.$id !== payloadMembership.$id),
    )
  }
}
